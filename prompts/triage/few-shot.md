...
-> (Route ([nix-implementer] (Some Interruption) (Some (Interruption [|staging rollout breaking production|] [|stop the staging rollout immediately; it is breaking production|]))))
...
-> (Route ([rust-auditor nix-implementer] None None))
...
-> (Escalate (AgentSpawnNeeded [|no registered agent owns documentation; the coordinator must decide whether to spawn one|]))
